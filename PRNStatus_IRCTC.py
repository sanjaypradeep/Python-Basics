...
while True:
    ...
    values = {'lccp_pnrno1': pnr_number, 'submit': 'Wait+For+PNR+Enquiry%21'}

    data = urllib.urlencode(values) #lccp_pnrno1=4803005210&submit=Wait%2BFor%2BPNR%2BEnquiry%2521
    print (data)
    ...
    if past_status != current_status:
        while True:
            try:
                print ("inside try..")
                # os.system(
                #     './sendsms.py ' + phone_number + ' "PNR #' + pnr_number + ' [Status Updated]      ' + sms_output[:-4] + '" -t ./sendsms.auth')
            except:
                pass
            else:
                break
